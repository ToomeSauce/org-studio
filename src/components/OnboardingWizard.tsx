...
import { ArrowRight, ArrowLeft, Plus, Sparkles, Check, Users, FolderKanban, Rocket } from 'lucide-react';
...
const STEP_LABELS = ['Organization', 'Team', 'Project', 'Done'];
...
export function OnboardingWizard({ onComplete }: OnboardingWizardProps) {
  ...
  const finish = async () => {
    setFinishing(true);
    try {
      // 1. Update settings (org name, mission, onboardingComplete)
      ...
      {STEP_LABELS.map((label, i) => (
        <div key={i} className="flex items-center gap-3">
          ...
          >
            ...
            <span className={`
              text-[13px] font-medium hidden sm:inline transition-colors duration-300
          ...
          {i < STEP_LABELS.length - 1 && (
            <div className={`
              ...
              ${i < step ? 'bg-[var(--success)]' : 'bg-[var(--border-default)]'}
            `} />
          )}
        </div>
      ))}
    ...
    <div className="max-w-xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-[var(--text-md)] text-[var(--text-tertiary)] leading-relaxed">
          Humans, AI agents, or both — build your roster.
        </p>
      </div>
      ...
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-default)] rounded-[var(--radius-lg)] p-5 space-y-4">
        ...
        {/* Type toggle */}
        <div>
          <label className="block text-[var(--text-xs)] font-medium text-[var(--text-tertiary)] mb-2 uppercase tracking-wider">Type</label>
          <div className="flex gap-2">
            <button
              onClick={() => setTmIsHuman(true)}
              className={`
                flex-1 py-2.5 rounded-[var(--radius-md)] text-[var(--text-sm)] font-medium border transition-all
                ${tmIsHuman
                  ? 'bg-[var(--accent-muted)] border-[var(--accent-primary)] text-[var(--accent-primary)]'
                  : 'bg-[var(--bg-primary)] border-[var(--border-default)] text-[var(--text-tertiary)] hover:border-[var(--border-strong)]'
                }
              `}
            >
              👤 Human
            ...
            <button
              onClick={() => setTmIsHuman(false)}
              className={`
                flex-1 py-2.5 rounded-[var(--radius-md)] text-[var(--text-sm)] font-medium border transition-all
                ${!tmIsHuman
                  ? 'bg-[var(--accent-2-subtle)] border-[var(--accent-2)] text-[var(--accent-2)]'
                  : 'bg-[var(--bg-primary)] border-[var(--border-default)] text-[var(--text-tertiary)] hover:border-[var(--border-strong)]'
                }
              `}
            >
        ...
        {/* Agent ID — only for agents */}
        {!tmIsHuman && (
          <div>
            <label className="block text-[var(--text-xs)] font-medium text-[var(--text-tertiary)] mb-1.5 uppercase tracking-wider">Agent ID</label>
            <input
              type="text"
              value={tmAgentId}
              onChange={e => setTmAgentId(e.target.value)}
              placeholder="e.g. my-agent"
              className="w-full bg-[var(--bg-primary)] border border-[var(--border-default)] rounded-[var(--radius-md)] px-3 py-2.5 text-[var(--text-sm)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--accent-primary)] transition-colors font-mono"
            />
            <p className="text-[var(--text-xs)] text-[var(--text-muted)] mt-1">
              This maps to your runtime agent ID (e.g., in OpenClaw)
            </p>
          </div>
        )}
          ...
          <div className="flex gap-2 flex-wrap">
            {EMOJI_OPTIONS.map(e => (
              <button
                ...
                className={`
                  w-10 h-10 rounded-[var(--radius-md)] text-lg flex items-center justify-center border transition-all
                  ${tmEmoji === e
                    ? 'border-[var(--accent-primary)] bg-[var(--accent-muted)] shadow-[0_0_12px_var(--accent-glow)]'
            ...
            <div key={i} className="flex items-center gap-3 bg-[var(--bg-secondary)] border border-[var(--border-default)] rounded-[var(--radius-md)] px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[var(--text-xs)] text-[var(--text-muted)] truncate">
                  {tm.title || (tm.isHuman ? 'Human' : 'Agent')}
                  {tm.domain ? ` · ${tm.domain}` : ''}
                </p>
              </div>
              <span className={`text-[var(--text-xs)] font-medium px-2 py-0.5 rounded-full ${tm.isHuman ? 'bg-[var(--warning-subtle)] text-[var(--warning)]' : 'bg-[var(--accent-2-subtle)] text-[var(--accent-2)]'}`}>
                {tm.isHuman ? 'Human' : 'Agent'}
              </span>
              <button
  ...
  const renderStep3 = () => (
    <div className="max-w-lg mx-auto text-center">
      ...
      <div className="space-y-3 text-left mb-10">
        ...
        {!orgName.trim() && teammates.length === 0 && !projName.trim() && (
      ...
      {step < 3 && (
        <div className="sticky bottom-0 bg-[var(--bg-primary)] border-t border-[var(--border-default)] px-6 py-4">
          <div className="max-w-xl mx-auto flex items-center justify-between">
            ...
            >
              {step === 2 ? (projName.trim() ? 'Review' : 'Skip & Review') : 'Continue'} <ArrowRight size={15} />
            </button>
          </div>
        </div>
      )}
