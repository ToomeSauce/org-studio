...
const phaseConfig: Record<string, { label: string; color: string }> = {
  active:   { label: 'Active',   color: 'text-[var(--success)] bg-[var(--success-subtle)] border-[rgba(52,211,153,0.3)]' },
  planning: { label: 'Planning', color: 'text-[var(--info)] bg-[var(--info-subtle)] border-[rgba(96,165,250,0.3)]' },
  paused:   { label: 'Paused',   color: 'text-[var(--warning)] bg-[var(--warning-subtle)] border-[rgba(251,191,36,0.3)]' },
    ...
    <div className={clsx(
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-[var(--text-md)] font-bold text-[var(--text-primary)] tracking-tight">{project.name}</h3>
        </div>
        <span className={clsx('text-[var(--text-xs)] font-semibold px-2.5 py-1 rounded-full border', phase.color)}>
          {phase.label}
        </span>
      </div>

      <p className="text-[var(--text-sm)] text-[var(--text-tertiary)] leading-relaxed mb-5">{project.description}</p>

      {/* Progress bar */}
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[var(--text-xs)] font-semibold text-[var(--text-muted)] uppercase tracking-wide">Progress</span>
          <span className="text-[var(--text-sm)] font-bold text-[var(--text-secondary)]">{completion}%</span>
        </div>
      ...
      {/* Task breakdown */}
      <div className="flex items-center gap-4 mb-5 text-[var(--text-xs)]">
      ...
      <div className="flex items-center justify-between pt-4 border-t border-[var(--border-subtle)]">
        <div className="flex items-center gap-4 text-[var(--text-xs)] text-[var(--text-muted)]">
        ...
        <Link href={`/tasks?project=${project.id}`}
          className="flex items-center gap-1.5 text-[var(--text-sm)] font-medium text-[var(--accent-primary)] hover:text-[var(--accent-hover)] transition-colors">
