...
export default function CronPage() {
  ...
  return (
    <div className="space-y-8">
      <Link href="/settings" className="text-sm text-[var(--text-muted)] hover:text-[var(--text-primary)] flex items-center gap-1 mb-2">
        <ArrowLeft size={14} />
        Settings
      </Link>
      <PageHeader
      ...
      {/* Gateway Notice */}
      {!cronData && (
        <GatewayNotice feature="cron jobs" />
      )}

      {/* Empty state */}
      {enabled.length === 0 && disabled.length === 0 && !cronData && (
        <div className="bg-[var(--card)] border border-[var(--border-default)] rounded-[var(--radius-lg)] p-8 text-center">
          <div className="text-4xl mb-3">⏰</div>
          <p className="text-[var(--text-base)] font-medium text-[var(--text-secondary)] mb-2">No cron jobs</p>
          <p className="text-[var(--text-sm)] text-[var(--text-muted)]">
            Cron jobs appear here when an agent runtime is connected. Set GATEWAY_URL in .env.local to get started.
          </p>
        </div>
      )}
