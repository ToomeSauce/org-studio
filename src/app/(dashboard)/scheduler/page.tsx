...
export default function SchedulerPage() {
  ...
  const runSweep = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        const triggered = (data.swept || []).filter((s: any) => s.triggered).length;
        const total = (data.swept || []).length;
        setSweepResult(triggered > 0
          ? `Swept ${total} agent${total !== 1 ? 's' : ''} — ${triggered} triggered`
          : total > 0
            ? `Swept ${total} agent${total !== 1 ? 's' : ''} — no triggers needed`
            : 'No enabled loops to sweep');
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      ...
      {/* Gateway Notice - info banner, not blocking */}
      {!gatewayStatus && (
        <div className="bg-[var(--bg-secondary)] border border-dashed border-[var(--border-default)] rounded-[var(--radius-lg)] p-4">
          <GatewayNotice feature="live scheduling" />
        </div>
      )}
      ...
      <div className="flex items-center gap-6 text-[var(--text-xs)]">
        ...
        {sweepResult && (
          <span className="text-[var(--text-muted)] italic">{sweepResult}</span>
        )}
        <div className="flex items-center gap-2 ml-auto">
          <button
            ...
            title="Run global sweep — check all agents for orphaned or stuck work"
          >
            <Zap size={12} className={sweeping ? 'animate-pulse' : ''} /> {sweeping ? 'Sweeping…' : 'Run Sweep'}
          </button>
          <button
