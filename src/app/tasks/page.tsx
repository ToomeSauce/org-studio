...
import { Plus, X, Circle, Bot, Activity } from 'lucide-react';
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5 text-[var(--text-xs)] text-[var(--text-muted)]">
          <span className="flex items-center gap-1.5">
            <Circle size={5} className={clsx('fill-current',
  ...
  return (
    <div className="w-[280px] shrink-0 border-l border-[var(--border-default)] bg-[var(--bg-secondary)] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {active.length === 0 && (
          <p className="text-[var(--text-sm)] text-[var(--text-muted)] text-center py-6">No active sessions</p>
        )}
        {active.map((s: any, i: number) => {
          ...
          return (
            <div key={s.key || i} className="px-3 py-2.5 rounded-[var(--radius-md)] bg-[var(--bg-primary)] border border-[var(--border-subtle)] hover:border-[var(--border-default)] transition-colors">
              <div className="flex items-center gap-2 mb-1">
                <Bot size={12} className="text-[var(--accent-primary)]" />
                <span className="text-[var(--text-xs)] font-medium text-[var(--text-primary)] truncate">{name}</span>
              </div>
              <div className="text-[var(--text-xs)] text-[var(--text-muted)] flex items-center gap-2">
                <span className={clsx(
                  'px-2 py-0.5 rounded-full text-[var(--text-xs)] font-semibold',
                  kind === 'cron' ? 'bg-[var(--warning-subtle)] text-[var(--warning)]'
                    : kind === 'run' ? 'bg-[var(--info-subtle)] text-[var(--info)]'
                    : 'bg-[var(--accent-muted)] text-[var(--accent-primary)]'
                )}>{kind}</span>
                {model && <span className="truncate font-[var(--font-mono)]">{model}</span>}
              </div>
            </div>
          );
        })}
        {Array.isArray(sessionList) && sessionList.filter((s: any) => !active.includes(s)).slice(0, 5).map((s: any, i: number) => {
          const name = s.displayName || s.label || s.key?.split(':').pop() || 'Session';
          return (
            <div key={`idle-${i}`} className="px-3 py-2 rounded-[var(--radius-md)] opacity-50">
              <div className="flex items-center gap-2">
                <Circle size={5} className="text-zinc-600 fill-current" />
                <span className="text-[var(--text-xs)] text-[var(--text-muted)] truncate">{name}</span>
              </div>
            </div>
          );
        })}
                    ...
                    {addingTo === col.key ? (
                      <div className="bg-[var(--bg-primary)] border border-[var(--border-strong)] rounded-[var(--radius-md)] p-3">
                        <input
                          ...
                          className="w-full text-[var(--text-sm)] bg-transparent text-[var(--text-primary)] placeholder-[var(--text-muted)] outline-none mb-2.5"
                          autoFocus
                          onKeyDown={e => {
                      ...
                      <button
                        ...
                        className="w-full py-2 text-[var(--text-xs)] text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] rounded-[var(--radius-md)] transition-colors flex items-center justify-center gap-1.5"
                      >
