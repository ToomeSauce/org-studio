...
function makeTeammates(): Teammate[] {
  return [
    ...
    },
    {
      ...
      domain: 'Catpilot Platform',
      owns: 'Platform code, CI/CD',
      defers: 'Infrastructure spend',
      description: 'Catpilot platform development',
      color: 'purple',
      isHuman: false,
    },
    {
...
describe('generateOrgMd', () => {
  ...
  it('includes team section listing all teammates', () => {
    ...
    expect(md).toContain('**Henry**');
    expect(md).toContain('**Ana**');
    expect(md).toContain('**Mikey**');
  ...
  it('marks humans and agents correctly in team roster', () => {
    ...
    // Basil is human
    expect(md).toContain('**Basil** 👤 (Human)');
    // Henry is an agent
    expect(md).toContain('**Henry** 📋 (Agent)');
  });
