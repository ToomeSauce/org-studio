...
describe('buildLoopPrompt', () => {
  it('builds prompt with default steps when no systemPrompt override', () => {
    const loop = makeLoop();
    const prompt = buildLoopPrompt(loop, 'Mikey');

    // Should contain the SCHEDULER_LOOP header
    expect(prompt).toContain('SCHEDULER_LOOP: autonomous work cycle for Mikey');
  ...
  it('uses systemPrompt override instead of default steps', () => {
    const loop = makeLoop({
  ...
  it('prepends global preamble when provided', () => {
    const loop = makeLoop();
    const preamble = 'IMPORTANT: Always read ORG.md before starting work.';
    const prompt = buildLoopPrompt(loop, 'Mikey', preamble);
  ...
  it('omits preamble when empty or whitespace', () => {
    const loop = makeLoop();
    const prompt = buildLoopPrompt(loop, 'Mikey', '   ');
  ...
  it('contains HEARTBEAT_OK idle suppression instructions', () => {
    const loop = makeLoop();
    const prompt = buildLoopPrompt(loop, 'Mikey');
  ...
  it('contains column workflow rules', () => {
    const loop = makeLoop();
    const prompt = buildLoopPrompt(loop, 'Mikey');
  ...
  it('contains "NEVER pull from planning" rule', () => {
    const loop = makeLoop();
    const prompt = buildLoopPrompt(loop, 'Mikey');
  ...
  it('only includes enabled steps in default mode', () => {
  ...
  it('interpolates agentId into activity status API calls', () => {
    const loop = makeLoop({ agentId: 'ana' });
    const prompt = buildLoopPrompt(loop, 'Ana');
