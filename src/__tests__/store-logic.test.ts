...
beforeEach(() => {
  store = {
    ...
    settings: {
      teammates: [
        { id: 'tm-1', agentId: 'mikey', name: 'Mikey', isHuman: false },
        { id: 'tm-2', agentId: 'ana', name: 'Ana', isHuman: false },
        { id: 'tm-3', agentId: '', name: 'Basil', isHuman: true },
      ],
    },
  };
});
...
describe('addTask', () => {
  ...
  it('sets initiatedBy from input', () => {
    const task = addTask({ title: 'Write docs', initiatedBy: 'mikey' });
    expect(task.initiatedBy).toBe('mikey');
  });
...
describe('hasActionableWork', () => {
  it('returns true when agent has backlog tasks (matched by name)', () => {
    addTask({ title: 'Backlog work', status: 'backlog', assignee: 'Mikey' });

    expect(hasActionableWork('mikey')).toBe(true);
  });

  it('returns true when agent has in-progress tasks', () => {
    addTask({ title: 'Active work', status: 'in-progress', assignee: 'Mikey' });

    expect(hasActionableWork('mikey')).toBe(true);
  });

  it('returns false when agent only has done tasks', () => {
    addTask({ title: 'Done work', status: 'done', assignee: 'Mikey' });

    expect(hasActionableWork('mikey')).toBe(false);
  });

  it('returns false when agent only has review tasks', () => {
    addTask({ title: 'Review work', status: 'review', assignee: 'Mikey' });

    expect(hasActionableWork('mikey')).toBe(false);
  });

  it('returns false when agent only has planning tasks', () => {
    addTask({ title: 'Planned work', status: 'planning', assignee: 'Mikey' });
  ...
  it('matches by agentId directly', () => {
    addTask({ title: 'Agent ID match', status: 'backlog', assignee: 'mikey' });
...
describe('trigger cooldown pattern', () => {
  it('debounces rapid triggers within the cooldown window', () => {
    ...
    expect(shouldTrigger('mikey', t0)).toBe(true);

    // Immediate second trigger should be debounced
    expect(shouldTrigger('mikey', t0 + 100)).toBe(false);

    // Trigger 30s later — still within cooldown
    expect(shouldTrigger('mikey', t0 + 30_000)).toBe(false);

    // Trigger after cooldown expires
    expect(shouldTrigger('mikey', t0 + 60_001)).toBe(true);
