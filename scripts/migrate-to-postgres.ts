...
async function createSchema(client: any): Promise<void> {
  ...
  await client.query(`
    CREATE TABLE IF NOT EXISTS org_studio_tasks (
      ...
      project_id TEXT REFERENCES org_studio_projects(id) ON DELETE SET NULL,
      assignee TEXT,
      priority TEXT,
      test_type TEXT,
      test_assignee TEXT,
...
async function readStore(storePath: string): Promise<StoreData> {
...
async function insertProjects(client: any, projects: Project[]): Promise<void> {
...
async function insertTasks(client: any, tasks: Task[]): Promise<void> {
...
async function insertSettings(client: any, settings: Record<string, any>): Promise<void> {
...
async function verifyCounts(client: any, expectedProjects: number, expectedTasks: number): Promise<boolean> {
  console.log('\nVerifying counts...');
