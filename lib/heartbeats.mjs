    ...
    try {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          // Log the actual outcome — `ok: true, skipped: true` is NOT a real
          // dispatch. Track `triggered` (set by the trigger handler when
          // fireOneShot enqueued) so silent fizzles surface in the journal.
          const dispatched = result.ok && result.triggered === true;
          const summary = dispatched
            ? 'dispatched'
            : `skipped (${result.reason || (result.ok ? 'unknown' : 'error')})`;
          console.log(`${TAG} Restart triggered for ${agentId}: ${summary}`);
        } catch (e) {
