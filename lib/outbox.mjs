...
async function drainBatch(baseUrl, apiKey) {
  ...
  try {
    ...
    // Note: outbox rows have workspace_id on them; process all workspaces (worker is global)
    const { rows } = await p.query(`
      SELECT id, idempotency_key, agent_id, payload, attempts, status, workspace_id
      ...
      try {
        ...
        const resp = await fetch(`${baseUrl}/api/outbox/drain`, {
          ...
          body: JSON.stringify({
            ...
            finalAttempt: row.attempts + 1 >= MAX_ATTEMPTS,
            payload: row.payload,
          }),
        });
