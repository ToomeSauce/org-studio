...
// #1650: read LAZILY (not module scope). This module is imported by server.mjs,
// and ESM evaluates imports BEFORE server.mjs's .env.local loader runs —
// module-scope reads froze these as '' even when .env.local set them.
const env = () => ({
  HEALTH_BOT_TOKEN: process.env.TELEGRAM_HEALTH_BOT_TOKEN || '',
  HEALTH_CHAT_ID: process.env.TELEGRAM_HEALTH_CHAT_ID || '',
  HEALTH_WEBHOOK_URL: process.env.TELEGRAM_HEALTH_ALERTS_WEBHOOK_URL || '',
  PUBLIC_URL: process.env.ORG_STUDIO_PUBLIC_URL || 'http://localhost:4501',
});
...
export function isHealthAlertsEnabled() {
  const { HEALTH_BOT_TOKEN, HEALTH_CHAT_ID, HEALTH_WEBHOOK_URL } = env();
...
export async function sendHealthAlert({ type, emoji, title, context }) {
  try {
    ...
    const { HEALTH_BOT_TOKEN, HEALTH_CHAT_ID, HEALTH_WEBHOOK_URL, PUBLIC_URL } = env();
